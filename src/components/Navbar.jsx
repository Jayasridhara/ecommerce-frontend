    ...
    const updateTime = () => {
      ...
      if (diff <= 60 * 1000) {
        ...
        if (!warningShown) {
          ...
          // option: you may not want alert in production
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-5 py-3 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
        ...
        {/* Shared Icons + Auth Links + Profile */}
        <div className="hidden md:flex items-center gap-6">
          
          {/* Conditionally show New Product button for seller */}
            ...
            <div className="flex items-center gap-4">
              <Link
                ...
                className={`relative hover:scale-105 transition-transform ${
                  wishlistCount === 0 ? "opacity-50 cursor-not-allowed pointer-events-none" : ""
                }`}
              >
              ...
              <Link
                to={cartCount > 0 ? "/cart" : "#"}
                className={`relative hover:scale-105 transition-transform ${
                  cartCount === 0 ? "opacity-50 cursor-not-allowed pointer-events-none" : ""
                }`}
              >
                <ShoppingCart className="w-6 h-6 text-blue-600" />
                {cartCount > 0 && (
          ...
          {/* My Orders Link */}
          {!hideNavIcons && isAuthenticated && currentPath !== "/orders" && (
            orders.length > 0 ? (
              <Link
                to="/orders"
                className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-medium hover:opacity-90 transition-all no-underline rounded-2xl"
              >
                My Orders
              </Link>
            ) : (
          ...
          {!isAuthenticated ? (
            <>
              <Link to="/login" className="text-gray-700 font-medium hover:text-blue-600 no-underline">
                Login
              </Link>
              <Link to="/register" className="bg-blue-500 text-white font-semibold px-5 py-2 rounded-full hover:bg-blue-600 no-underline">
                Register
              </Link>
            </>
          ) : (
            ...
            {!isAuthenticated ? (
              ...
              <>
                <div className="flex flex-col gap-2">
                  ...
                  {!hideNavIcons && currentPath !== "/orders" && (
                    <Link
                      to="/orders"
                      onClick={() => setMobileMenuOpen(false)}
